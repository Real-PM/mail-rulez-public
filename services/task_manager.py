...
class TaskManager:
    ...
    def add_account(self, account_config: AccountConfig) -> bool:
        ...
        with self._lock:
            ...
            try:
                # Create the processor - this is synchronous and completes before returning
    ...
    def get_aggregate_stats(self) -> Dict[str, Any]:
        ...
        if not self._initialized:
            ...
            # Use lock to ensure thread-safe access to processors during initialization
            with self._lock:
                account_count = len(self.processors)
            return {
                'total_accounts': account_count,  # Show actual account count even during init
    ...
    def load_accounts_from_config(self):
        ...
        try:
            ...
            # Track successful account loads
            loaded_count = 0
            failed_count = 0
            
            for account_config in self.config.accounts:
                if self.add_account(account_config):
                    loaded_count += 1
                else:
                    failed_count += 1
            
            self.logger.info(f"Loaded {loaded_count} accounts from configuration (failed: {failed_count})")
            
            # Only mark as initialized after all accounts are loaded
            # This ensures get_aggregate_stats() won't return partial data
    ...
    def is_initialized(self) -> bool:
        """
        Check if task manager is fully initialized
        
        Returns:
            bool: True if initialization is complete
        """
...
def get_task_manager() -> TaskManager:
    ...
    with _task_manager_lock:
        if _task_manager is None:
            ...
            # Load accounts from configuration synchronously
            # This ensures all accounts are loaded before returning
